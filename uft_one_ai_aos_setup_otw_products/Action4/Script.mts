...
AIUtil.FindTextBlock(DataTable.Value("Product Name", "Setup_New_Speaker_Product")).Click
AIUtil.FindTextBlock("CUSTOMISATION").Click
If AIUtil("camera", micAnyText, micFromBottom, 1).Exist(3) Then
...
WpfWindow("AOS Managment Console").Dialog("Select image for this color").WinEdit("File name:").Set Environment.Value("TestDir") & "\Kyber Crystals\" 
AIUtil("button", "Open").Click
WpfWindow("AOS Managment Console").Dialog("Select image for this color").WinEdit("File name:").Set DataTable.Value("Image Name", "Setup_New_Speaker_Product")

'AIUtil.FindText(DataTable.Value("Image Name", "Setup_New_Speaker_Product")).Click
AIUtil("button", "Open").Click

If (AIUtil("check_box", "GENERAL IMAGES").Exist(3) = FALSE) Then
	AIUtil("check_box", "", micWithAnchorOnLeft, AIUtil("camera", micAnyText, micFromBottom, 1)).SetState "On"
Else
	AIUtil("check_box", "GENERAL IMAGES").SetState "On"
End If
